...
-   There are multiple course IDs

-   There are no grade categories

-   Is final grade points earned divided by points possible?
...
-   Rows are typically individuals or unique instances

What columns typically (or you think may) represent:

-   Columns are typically different entities that the individuals are
    partaking in or data that is collected about them
...
-   The number of rows was reduced when changing the filter from 70% to
    80%
...
```{r}
d |> 
  select(student_id, course_id, FinalGradeCEMS)|>
  filter(FinalGradeCEMS > 75)|>
  arrange(desc(FinalGradeCEMS))
